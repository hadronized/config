...
def create_right_prompt [] {
    ...
    ] | str join | str replace --regex --all "([/:])" $"(ansi black)${1}(ansi cyan)")
    ...
    if ($git_output.exit_code) == 0 {
      ...
      if $battery_percent > 80 {
        $battery_segment = $"(ansi reset)(ansi green)  ($battery_percent)%"
...
$env.PATH = ($env.PATH | split row (char esep) | append ["/usr/local/bin", "~/.local/bin", "~/.cargo/bin"])
